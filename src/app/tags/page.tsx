...
function TagsContent() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold tracking-tight text-on-surface">标签</h1>
        <p className="mt-1 text-[13px] text-on-surface-variant">按标签浏览文章</p>
      </div>
            ...
            <Link key={post.slug} href={`/blog/${post.slug}`} className="group block no-underline">
              <div className="rounded-2xl border border-outline-variant/40 bg-surface p-5 card-hover">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {post.tags.map(t => (
                    <span key={t} className="tag-chip text-[11px]">{t}</span>
                  ))}
                </div>
              </div>
            </Link>
