...
export default function ArchivePage() {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold tracking-tight text-on-surface">归档</h1>
        <p className="mt-1 text-[13px] text-on-surface-variant">共 {totalPosts} 篇文章</p>
      </div>

      <div className="space-y-10">
        {archive.map(({ year, months }) => (
          <section key={year}>
            <h2 className="mb-4 text-xl font-bold text-primary">{year}</h2>
            <div className="space-y-4">
              {months.map(({ month, posts }) => (
                <div key={`${year}-${month}`}>
                  ...
                  <div className="ml-6 space-y-2 border-l border-outline-variant/40 pl-4">
                    {posts.map(post => (
                      <div key={post.slug} className="relative">
                        <div className="absolute -left-[21px] top-2 h-2 w-2 rounded-full border-2 border-primary bg-surface" />
                        <Link
                          href={`/blog/${post.slug}`}
                          className="group block rounded-lg p-2 transition-colors hover:bg-surface-container-high no-underline"
                        >
                          <span className="text-[13px] font-medium text-on-surface group-hover:text-primary transition-colors duration-300">
                            {post.title}
                          </span>
