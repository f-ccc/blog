...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  const isLoginPage = pathname === '/fc/login'
    ...
    <div className="flex min-h-[calc(100vh-4rem)]">
      <aside className="hidden w-60 shrink-0 border-r border-outline-variant bg-surface md:block">
        ...
        <div className="absolute bottom-4 px-3 w-60">
          <Link href="/" className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-on-surface-variant hover:bg-surface-container-high transition-colors no-underline">
            <LogOut size={18} />
            返回前台
          </Link>
        </div>
...
function NavItem({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) {
  return (
    <Link href={href} className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-on-surface-variant hover:bg-surface-container-high hover:text-on-surface transition-colors no-underline">
      {icon}{label}
    </Link>
  )
}
...
function MobileNavItem({ href, label }: { href: string; label: string }) {
  return (
    <Link href={href} className="rounded-lg px-3 py-1.5 text-xs font-medium text-on-surface-variant hover:bg-surface-container-high transition-colors no-underline">
      {label}
    </Link>
  )
}
